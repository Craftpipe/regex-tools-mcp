...
export function registerValidatePattern(server: McpServer): void {
  server.tool(
    "validate_pattern",
    "Validates that a regex pattern is syntactically correct and checks for common mistakes such as unbalanced groups, catastrophic backtracking risks, or deprecated syntax. Use this before embedding a pattern in production code.",
    {
    ...
    async (params) => {
      try {
        ...
        const flags: string = typeof params.flags === 'string' ? params.flags : '';
        const strict: boolean = typeof params.strict === 'boolean' ? params.strict : false;
        ...
        if (pattern.length > 0 && syntaxValid) {
          ...
          if (/\([^)]*[+*][^)]*\)[+*?]/.test(pattern)) {
            warnings.push(
              "Potential catastrophic backtracking: nested quantifiers detected (e.g. '(a+)+' or '(a*)*'). " +
          ...
          if (overlapMatch) {
            ...
            if (branchSet.size < branches.length) {
              warnings.push(
                `Overlapping alternation branches inside a quantified group: '${overlapMatch[0]}'. ` +
          ...
          if (/\.\*|\.\+/.test(pattern) && strict) {
            warnings.push(
              "Unbounded wildcard '.*' or '.+' found. Consider using a more specific character class to avoid over-matching or performance issues."
            );
          }
          ...
          if (strict) {
            ...
            while ((m = singleCharClass.exec(pattern)) !== null) {
              suggestions.push(
                `Redundant character class '[${m[1]}]' can be simplified to '${m[1]}'`
              );
            }
          }
          ...
          if (backrefs) {
            ...
            for (const ref of backrefs) {
              ...
              if (refNum > capturingGroups) {
                errors.push(
                  `Backreference '${ref}' refers to group ${refNum}, but only ${capturingGroups} capturing group(s) exist`
                );
              }
            }
          }
          ...
          if (strict && !pattern.startsWith('^') && !pattern.endsWith('$')) {
            suggestions.push(
              "Pattern has no anchors ('^' or '$'). If you intend to match the full string, consider adding anchors to avoid partial matches."
            );
          }
          ...
          if (seenFlags.has('i') && /[a-z]-[a-z]|[A-Z]-[A-Z]/i.test(pattern)) {
            warnings.push(
              "Case-insensitive flag 'i' is set, but the pattern contains explicit case ranges in character classes. " +
          ...
          if (!seenFlags.has('u') && !seenFlags.has('v')) {
            if (/\\u\{[0-9a-fA-F]+\}/.test(pattern)) {
              errors.push(
                "Unicode escape '\\u{...}' requires the 'u' or 'v' flag to be set"
              );
            }
            if (/\\p\{/.test(pattern) || /\\P\{/.test(pattern)) {
              errors.push(
                "Unicode property escapes '\\p{...}' or '\\P{...}' require the 'u' or 'v' flag to be set"
              );
            }
          }
          ...
          if (!seenFlags.has('s') && strict) {
            if (/\./.test(pattern)) {
              suggestions.push(
                "The '.' metacharacter does not match newlines by default. If you need to match newlines, add the 's' (dotAll) flag or use '[\\s\\S]'."
              );
            }
          }
          ...
          if (zeroWidthQuantifier.test(pattern)) {
            warnings.push(
              "Quantifier applied to a zero-width assertion (e.g. '^+', '\\b*', lookahead+). This is likely a mistake."
            );
          }
