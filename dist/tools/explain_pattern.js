...
function describeMetachar(value, flags) {
...
function describeQuantifier(min, max, lazy, base) {
    let range;
    if (min === 0 && max === Infinity)
        range = 'zero or more times';
    else if (min === 1 && max === Infinity)
        range = 'one or more times';
    else if (min === 0 && max === 1)
        range = 'zero or one time (optional)';
    else if (min === max)
        range = `exactly ${min} time${min !== 1 ? 's' : ''}`;
    else if (max === Infinity)
        range = `at least ${min} time${min !== 1 ? 's' : ''}`;
    else
        range = `between ${min} and ${max} times`;
...
function describeCharClass(negated, content) {
    const prefix = negated ? 'any character NOT in the set: ' : 'any one character from the set: ';
    return prefix + `[${content}]`;
}
function describeGroup(kind, content) {
    if (kind === 'capturing')
        return `a capturing group matching: ${content}`;
    if (kind === 'non-capturing')
        return `a non-capturing group matching: ${content}`;
    if (kind.startsWith('named'))
        return `${kind} matching: ${content}`;
    return `a group (${kind}) matching: ${content}`;
}
function describeLookaround(kind, content) {
    const descriptions = {
...
function buildBreakdown(tokens, flags) {
...
function buildSummary(pattern, flags, breakdown) {
    ...
    if (hasAnchors)
        base += ', anchored to specific positions in the string';
    if (hasGroups)
        base += ', with one or more captured groups';
    if (hasQuantifiers)
        base += ', using quantifiers to control repetition';
    base += '.';
    return base;
}
function collectWarnings(pattern, flags, tokens) {
    const warnings = [];
    ...
    const hasCapturing = tokens.some((t) => t.type === 'group' && t.kind === 'capturing');
...
function explainPattern(pattern, flags, verbosity) {
...
export function registerExplainPattern(server) {
    server.tool('explain_pattern', 'Explains a regex pattern in plain English, breaking it down token by token. Helps developers understand what an existing pattern does without having to decode it manually.', {
        ...
        verbosity: z
            .string()
            .optional()
            .describe("Level of detail: 'brief' for a one-sentence summary, 'normal' for a full breakdown, 'detailed' for an exhaustive token-by-token analysis. Defaults to 'normal'."),
    }, async (params) => {
        try {
            ...
            const verbosityRaw = typeof params.verbosity === 'string' ? params.verbosity.toLowerCase().trim() : 'normal';
            const verbosity = verbosityRaw === 'brief' || verbosityRaw === 'detailed' ? verbosityRaw : 'normal';
            ...
            const result = explainPattern(pattern, flags, verbosity);
            return {
                content: [
                    {
                        type: 'text',
